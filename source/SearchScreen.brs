...
Function getSearchHistory() As Object

    ' Get Saved Search History
    searchHistory = RegUserRead("searchHistory")

    ' Make sure it isn't empty
    if searchHistory = invalid then
        historyList = CreateObject("roArray", 1, true)
    else
        historyList = searchHistory.tokenize("|")
    end if
...
Function addSearchHistory(searchTerm As String) As Boolean
    searchHistory = getSearchHistory()

    ' Look For Term In List Already And Remove
    index = 0
    for each term in searchHistory
        if searchTerm = term then
            searchHistory.Delete(index)
            exit for
        end if
        index = index + 1
    end for

    ' Remove Last Search Term If More Then 8
    ...
    ' Save History
    str = SearchHistoryArrayToString(searchHistory)
    RegUserWrite("searchHistory", str)
...
'**********************************************************
'** Change Search History Array To String
'**********************************************************

Function SearchHistoryArrayToString(array As Object) As String
    str = ""
    arraySize = array.Count() - 1
    for count = 0 to arraySize
        str = str + tostr(array[count]) + "|" 
    end for
